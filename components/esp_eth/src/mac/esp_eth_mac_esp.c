...
typedef struct {
    ...
    esp_os_task_notify_t rx_task_notify;
...
{
    esp_os_unlock_mutex(&emac->multi_reg_mutex);
    return ESP_OK;
}
...
static void emac_esp_free_driver_obj(emac_esp32_t *emac)
{
    ...
    if (sleep_retention_is_module_inited(module)) {
        ...
        /* emac was allocated via heap_caps_calloc() or calloc() depending on
         * ETH_MAC_FLAG_WORK_WITH_CACHE_DISABLE; heap_caps_free() handles both.
         */
        heap_caps_free(emac);
    }
}
...
esp_eth_mac_t *esp_eth_mac_new_esp32(const eth_esp32_emac_config_t *esp32_config, const eth_mac_config_t *config)
{
    ...
    /* NuttX: route through esp_os_intr_alloc() so that NuttX's
     * g_handle_map[cpu][irq] is populated by esp_setup_irq_with_flags_intrstatus().
     * esp_os_intr_alloc() already calls up_enable_irq() internally, so no
     * separate esp_intr_enable() is required here.
     */
